...
func (m *SnippetModel) GetSecondBreakfast(id int) (Snippet, error) {
	//am scos 'title'
	stmt := `SELECT id, content, created, expires FROM snippets
    WHERE expires > UTC_TIMESTAMP() AND id = ?`
...
func (m *SnippetModel) Latest() ([]Snippet, error) {
	...
	// returns a sql.Rows result set containing the result
	rows, err := m.DB.Query(stmt)
	if err != nil {
		return nil, err
	}

	defer rows.Close() // nvr forget!!
